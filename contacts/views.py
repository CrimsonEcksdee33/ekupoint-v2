...
def generate_pdf(request): 
    ...
    for u in all_users:
        # Calculate total 'Aktivitas' points for this specific user.
        aktivitas_points = Aktivitas.objects.filter(user=u).aggregate(
            total=Sum(F('aturan_merit__poin') * F('kuantitas'), default=0)
        )['total']
        ...
        # Calculate the final total points, starting with a base of 100.
        total_points = (aktivitas_points or 0) - (pelanggaran_points or 0) + 100
    ...
    for item in user_rekap_list:
        line = f"User: {item['user_obj'].username}, Aktivitas: {item['aktivitas'] or 0}, Pelanggaran: {item['pelanggaran'] or 0}, Total: {item['total']}"
    ...
    c.save()
    buf.seek(0)
    return FileResponse(buf, as_attachment=True, filename='ekupoint_report.pdf')
...
def generate_user_pdf(request, user_id): 
    # variables
    user_obj = get_object_or_404(User, id=user_id)
    aktivitas_records = Aktivitas.objects.order_by('-created_at').filter(user__id=user_id)
    pelanggaran_records = Pelanggaran.objects.order_by('-created_at').filter(user__id=user_id)
    ...
    textob.textLine(f"Laporan Aktivitas dan Pelanggaran bagi {user_obj.get_full_name()  }")
    textob.textLine(f"Nama: {user_obj.get_full_name()}")
    textob.textLine("")

    textob.textLine("Aktivitas:")
    for akt in aktivitas_records:
        line = f"- {akt.aturan_merit.bidang} | {akt.aturan_merit.aktivitas} | Poin: {akt.aturan_merit.poin} | Kuantitas: {akt.kuantitas}"
        textob.textLine(line)

    textob.textLine("")
    textob.textLine("Pelanggaran:")
    for pel in pelanggaran_records:
        line = f"- {pel.aturan_demerit.pelanggaran} | Poin: {pel.aturan_demerit.poin} | Kuantitas: {pel.kuantitas}"
        textob.textLine(line)
    ...
    c.save()
    buf.seek(0)
    return FileResponse(buf, as_attachment=True, filename=f'Laporan Aktivitas {user_obj.get_full_name()}.pdf')
...
def generate_user_table_pdf(request, user_id=None):
    ...
    # Aktivitas table
    # styles = getSampleStyleSheet()
    small = ParagraphStyle('small', parent=styles['Normal'], fontSize=8, leading=10, fontName='Times-Roman', splitLongWords=True)
    ...
    sig_left_content = [
        ...
        Spacer(1, 80),  # 60-point gap for signature
